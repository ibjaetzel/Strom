...
class ChargingVisualization extends IPSModule
{
    ...
    public function RequestAction($Ident, $Value)
    {
        switch ($Ident) {
            case 'WallboxMode':
                $this->SetConfiguredValue('WallboxModeVariable', $Value);
                break;
            case 'WallboxSocMin':
                $this->SetConfiguredValue('WallboxSocMinVariable', $Value);
                break;
            case 'HomeGridChargeEnabled':
                $this->SetConfiguredValue('HomeGridChargeEnabledVariable', $Value ? true : false);
                break;
            case 'HomeWindow1':
                $this->SetWindowValues(1, $Value);
                break;
            case 'HomeWindow2':
                $this->SetWindowValues(2, $Value);
                break;
            case 'Refresh':
                break;
        }
    ...
    private function SetWindowValues(int $index, $value): void
    {
        ...
        if (isset($value['start'])) {
            $this->SetConfiguredValue($prefix . 'StartVariable', $value['start']);
        }
        if (isset($value['end'])) {
            $this->SetConfiguredValue($prefix . 'EndVariable', $value['end']);
        }
        if (isset($value['targetSoc'])) {
            $this->SetConfiguredValue($prefix . 'SocVariable', $value['targetSoc']);
        }
    }
    ...
    private function BuildPayload(): array
    {
        ...
        $outGroups = [];
        foreach ($groups as $g) {
            $varId = intval($g['VariableID'] ?? 0);
            $dailyId = intval($g['DailyVariableID'] ?? 0);
            $outGroups[] = [
                'name' => strval($g['Name'] ?? ''),
                'value' => ($varId > 0 && @IPS_VariableExists($varId)) ? @GetValue($varId) : 0,
                'daily' => ($dailyId > 0 && @IPS_VariableExists($dailyId)) ? @GetValueFormatted($dailyId) : '',
                'icon' => strval($g['Icon'] ?? 'plug')
            ];
        }

        $stats = [];
        foreach ([
            'DayProduction' => 'PV heute',
            'WeekProduction' => 'PV Woche',
        ...
        return [
            'pvSouth' => $this->V('PVSouth'),
            'pvWest' => $this->V('PVWest'),
            'batteryPower' => $this->V('BatteryPower'),
            'batterySoc' => $this->V('BatterySoC'),
            'l1' => $this->V('L1'),
            'l2' => $this->V('L2'),
            'l3' => $this->V('L3'),
            'groups' => $outGroups,
            'stats' => $stats,
            'wallbox' => [
                'power' => $this->V('WallboxPower'),
                'mode' => intval($this->V('WallboxModeVariable', 0)),
                'socMin' => floatval($this->V('WallboxSocMinVariable', 80))
            ],
            'homeBattery' => [
                'gridChargeEnabled' => boolval($this->V('HomeGridChargeEnabledVariable', false)),
                'windows' => [
...
// Kompatibilitätsalias, falls IP-Symcon oder ein Testskript noch den alten Klassennamen verwendet.
if (!class_exists('SolarFlowEnergy')) {
    class SolarFlowEnergy extends ChargingVisualization {}
}

// Kompatibilität: Einige IP-Symcon Lade-/Testpfade rufen den Modulnamen als Funktion auf.
// Dann erzeugen wir hier explizit die Modulinstanz statt mit "undefined function" abzubrechen.
function ChargingVisualization($InstanceID = 0)
{
    return new ChargingVisualization($InstanceID);
}

function SolarFlowEnergy($InstanceID = 0)
{
    return new SolarFlowEnergy($InstanceID);
}

function SFV_Refresh($id)
{
